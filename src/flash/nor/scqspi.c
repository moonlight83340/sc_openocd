...
static int flash_checksum_memory(struct flash_bank *bank, uint32_t offset, uint32_t count,
				 uint32_t *crc)
{
	int ret;
	uint32_t checksum = 0;
	...
	ret = image_calculate_checksum(flash_buf, count, &checksum);
	if (ret != ERROR_OK) {
		goto clean;
	}
			 ...
			 uint32_t count)
{
	...
	ret = flash_checksum_memory(bank, offset, count, &actual_crc);
	if (ret != ERROR_OK) {
		goto end;
	}
